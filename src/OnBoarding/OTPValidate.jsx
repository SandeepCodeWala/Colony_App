...
import OTPTextView from 'react-native-otp-textinput'; // npm i react-native-otp-textinput
import { showToast } from '../services/Toast';
import { postApi } from '../services/network/api';

export default function OTPValidate({ navigation, route }) {
  const { dummyotp, phone } = route?.params || {};
  ...
  const handleVerify = async () => {
    ...
    let data = { phone: phone, code: otp };
console.log("THIS IS VERIFY OTP data==",data)
...
console.log("THIS IS VERIFY OTP RESPONSE+++",response)
    ...
    // setTimeout(() => {
    //   setIsLoading(false);
    //   showToast('error', 'OTP Verified Successfully!');
    //   navigation.replace('Login'); // Replace with your next screen
    // }, 1500);
  };
